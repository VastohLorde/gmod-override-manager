...
class App(tk.Tk):
    def __init__(self):
        ...
        self.packs = []
        self.title("GMod Override Manager - No Community")
    ...
    TUTORIAL = (
        "GMOD OVERRIDE MANAGER â€” QUICK TUTORIAL\n"
        "======================================\n\n"
        ...
        "   Changes apply on the next map load or server RECONNECT â€”\n"
        ...
        "5) INCLUDED PACKS\n"
        "   This No Community version does not connect to GitHub.\n"
        "   It already includes approved packs in overrides/.\n"
        "   Select one, click Enable, then reconnect to GMod.\n\n"
        "6) WHO SEES IT\n"
    ...
    def refresh(self):
        ...
        if not ok:
            self.note.set("GMod 'addons' folder not found â€” set the correct GMod folder above.")
        elif not self.packs:
    ...
    def update_desc(self):
        ...
        if p:
            ...
            self.desc.set(f"{p['name']} â€” overrides {p['character']} ({p['skin']}).  {d}")
    ...
    def lt_enable(self):
        messagebox.showinfo("No Community Version", "Translator tools are not included in this offline build.")
